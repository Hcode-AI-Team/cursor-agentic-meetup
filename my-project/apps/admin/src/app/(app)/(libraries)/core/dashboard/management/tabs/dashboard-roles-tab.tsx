...
type DashboardRoleDraftPayload = {
  values: RoleFormData;
};

const DASHBOARD_ROLE_DRAFT_STORAGE_KEY = 'core-dashboard-role-form-draft';
...
export function DashboardRolesTab() {
  ...
  const {
    ...
    isLoading,
    refetch,
  } = useQuery<{ data: DashboardRole[]; total: number }>({
    queryKey: [
      'dashboard-roles',
      page,
      pageSize,
    ...
    queryFn: async () => {
      ...
      const response = await request<{ data: DashboardRole[]; total: number }>({
        url: `/dashboard-role?${params.toString()}`,
        method: 'GET',
      });
      return response.data;
    },
  ...
  } = useFormDraft<DashboardRoleDraftPayload>({
    storageKey: DASHBOARD_ROLE_DRAFT_STORAGE_KEY,
    value: {
      values: {
  ...
  const handleDeleteConfirm = async () => {
    ...
    try {
      await request({
        url: `/dashboard-role/${roleToDelete}`,
        method: 'DELETE',
      });
  ...
  return (
    <div className="space-y-4">
      <div className="flex justify-between items-center">
        <div>
          <h2 className="text-lg font-semibold">{t('dashboardRolesTab')}</h2>
          <p className="text-muted-foreground text-sm">
            {t('manageDashboardRoles')}
          </p>
        </div>
        ...
        <Table>
          ...
          <TableBody>
            {isLoading ? (
              ...
              roles.map((role) => {
                const dashboardName =
                  role.dashboard.dashboard_locale.find(
                    (l) => l.locale.code === currentLocaleCode
                  )?.name || role.dashboard.slug;
